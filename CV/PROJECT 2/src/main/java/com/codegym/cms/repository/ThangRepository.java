...
public interface ThangRepository extends PagingAndSortingRepository<Thang, Long> {

    Iterable<Thang> findAllByIsDeletedEquals(int isDeleted);

    @Transactional
    @Modifying
    @Query("UPDATE Thang b SET b.isDeleted = 1, b.deleted_at = :deleted_at, b.deleted_by = :deleted_by WHERE b.id=:id")
    void softDelete(@Param("deleted_at") LocalDate deleted_at, @Param("deleted_by") String deleted_by, @Param("id") Long id);


    @Transactional
    @Modifying
    @Query(value = "INSERT INTO Thang ( id, name, class1, value, luachon, innerText, created_at, created_by )" + 
               "VALUES ((SELECT IFNULL((SELECT MAX(id) FROM Thang C) + 1, 1 )), :name, :class1, :value, :luachon, :innerText, :created_at, :created_by);", nativeQuery = true)
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE Thang SET name = :name, class1 = :class1, value = :value, luachon = :luachon, innerText = :innerText, updated_at = :updated_at, updated_by = :updated_by WHERE id = :id", nativeQuery = true)
