...
public class BRepository {
    public void createInterFace() throws SQLException, ClassNotFoundException {
        ...
        for (int i = 0; i < dtp.tableSize(); i++) {
            ...
            String question = "com.codegym.cms.repository; \n";
            if(  dtp.pkSize(0)>0){
            ...
            if(  dtp.pkSize(0)>0){
                for (int j= 0;j < dtp.pkSize(i);j++){
                    question += "    Iterable<"+dtp.table(i)+"> findAllBy"+dtp.table(i)+"AndIsDeletedEquals("+ dtp.pk(i,j)+ "  " + dtp.pk(i,j).toLowerCase()+", int isDeleted);\n" +
                            "\n" +
                            "\n";
                }
            ...
            question+=", @Param(\"updated_at\") LocalDate updated_at, @Param(\"updated_by\") String updated_by, @Param(\"id\") Long id);\n" +
                    "\n" +
                    "}\n";
